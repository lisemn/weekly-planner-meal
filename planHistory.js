// planHistory.js
// Archives the previously saved weekly meal plan whenever a new one is saved,
...
// Wrap the existing saveCurrentPlan so the previous plan is archived first.
(function installPlanHistoryHook() {
  if (typeof saveCurrentPlan !== "function") {
    console.warn("planHistory.js: saveCurrentPlan not found. Make sure planHistory.js is loaded AFTER app.js.");
    return;
  }
  const originalSaveCurrentPlan = saveCurrentPlan;
  window.saveCurrentPlan = function patchedSaveCurrentPlan(newPlan) {
    archiveCurrentPlanToHistory();
    return originalSaveCurrentPlan(newPlan);
  };
})();

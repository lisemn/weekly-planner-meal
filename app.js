...
function cloneMealForPlan(meal) {
  return { id: meal.id, name: meal.name, type: meal.type };
}
  ...
  return `
    <div class="meal-row">
      ...
      <span class="meal-content">
        ...
        <span class="meal-meta">${formatMoney(resolvedMeal.cost)} · ${formatCalories(resolvedMeal.calories)}</span>
      ...
      <span class="meal-actions">${reviewButton}${customiseButton}</span>
    </div>
  `;
}
...
function parseRecipeIngredients(text) {
  return text
    ...
    .filter(Boolean)
    .map((line) => {
      const [name, qty, unit, category] = line.split(",").map((part) => part.trim());
      return {
...
function ingredientsToText(ingredients = []) {
  return ingredients.map((item) => `${item.name}, ${item.qty}, ${item.unit}, ${item.category}`).join("\n");
}
...
function getShoppingItems(mealTypeFilter = "all") {
  ...
  return Array.from(map.values()).map((item) => ({ ...item, sources: Array.from(item.sources) }));
}
...
function renderShoppingList() {
  ...
  if (query) {
    items = items.filter((item) => {
      ...
      return `${item.name} ${item.category} ${sourceText}`.toLowerCase().includes(query);
    });
  }
          ...
          .map((item) => `
            <div class="shopping-item">
              ...
              <div class="shopping-item-main">
                ...
                <p>${formatNumber(item.qty)} ${escapeHTML(item.unit)}</p>

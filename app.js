...
function ensureUserShape(user, username) {
  ...
  user.mealPlans.history = Array.isArray(user.mealPlans.history) ? user.mealPlans.history : [];
  user.mealPlans.lockedSlots = Array.isArray(user.mealPlans.lockedSlots) ? user.mealPlans.lockedSlots : [];
  user.customRecipes = user.customRecipes || [];
  user.favouriteRecipeIds = Array.isArray(user.favouriteRecipeIds) ? user.favouriteRecipeIds : [];
  user.excludedRecipeIds = Array.isArray(user.excludedRecipeIds) ? user.excludedRecipeIds : [];
  user.leftoverIngredients = Array.isArray(user.leftoverIngredients) ? user.leftoverIngredients : [];
...
// ==================== Exclude Recipes From Future Plans ====================

function getExcludedRecipeIds() {
  const user = getCurrentUserData();
  return user?.excludedRecipeIds || [];
}

function isRecipeExcluded(meal) {
  if (!meal?.id) return false;
  return getExcludedRecipeIds().includes(meal.id);
}

function toggleRecipeExclusion(recipeId) {
  const meal = getAllMealItems().find((item) => item.id === recipeId);
  if (!meal) return;

  let excluded = false;
  const saved = saveCurrentUserData((user) => {
    user.excludedRecipeIds = user.excludedRecipeIds || [];
    if (user.excludedRecipeIds.includes(recipeId)) {
      user.excludedRecipeIds = user.excludedRecipeIds.filter((id) => id !== recipeId);
      excluded = false;
    } else {
      user.excludedRecipeIds.push(recipeId);
      excluded = true;
    }
  });
  ...
  const plan = getCurrentPlan();
  if (plan) renderMealPlan(plan);
  showSuccess("recipe-success", excluded ? `${meal.name} will be excluded from future generated plans.` : `${meal.name} can be used in future generated plans again.`);
}

function getFilteredMeals(type, preferences) {
  const meals = getMealsByType(type).filter((meal) => !isRecipeExcluded(meal));
...
function renderMealItem(day, type, label, meal) {
  ...
  const reviewButton = getCurrentUser()
    ...
    : "";
  const excludeButton = getCurrentUser() && !viewingSharedPlan && resolvedMeal.id
    ? `<button class="btn btn-outline btn-xs" onclick="toggleRecipeExclusion('${resolvedMeal.id}')">${isRecipeExcluded(resolvedMeal) ? "Include" : "Exclude"}</button>`
    : "";
  ...
  return `
    <div class="meal-row ${completed ? "meal-completed" : ""}">
      ...
      <span class="meal-actions">${completionButton}${reviewButton}${excludeButton}${customiseButton}</span>
    </div>
  `;
}
...
function renderCustomizeOptions() {
  ...
  let meals = getMealsByType(customizeState.type)
    .filter((meal) => !isRecipeExcluded(meal))
    .filter((meal) => mealMatchesPreferences(meal, preferences, safeOnly));
...
function getFavourites() {
  const user = getCurrentUserData();
  return Array.isArray(user?.favouriteRecipeIds) ? user.favouriteRecipeIds : [];
}

function saveFavourites(recipeIds = []) {
  const uniqueIds = Array.from(new Set(recipeIds.filter(Boolean).map(String)));
  return saveCurrentUserData((user) => {
    user.favouriteRecipeIds = uniqueIds;
  });
}

function isFavourite(recipeId) {
  return getFavourites().includes(String(recipeId));
}
...
function addFavourite(recipeId) {
  ...
  return saveFavourites([...ids, id]);
}

function removeFavourite(recipeId) {
  const id = String(recipeId || "");
  return saveFavourites(getFavourites().filter((item) => item !== id));
}
...
function toggleFavourite(recipeId) {
  ...
  if (isFavourite(recipeId)) removeFavourite(recipeId);
  else addFavourite(recipeId);
...
function renderFavourites() {
  ...
  let meals = getFavouriteMeals();

  if (type !== "all") meals = meals.filter((meal) => meal.type === type);
  if (query) meals = meals.filter((meal) => mealSearchText(meal).includes(query));

  const count = document.getElementById("favourites-count");
  if (count) count.textContent = `${getFavouriteMeals().length} favourite recipe${getFavouriteMeals().length === 1 ? "" : "s"} saved`;

  if (meals.length === 0) {
    container.innerHTML = '<p class="empty-state small-empty">No favourite recipes match the current filter. Use the star button in Recipe Library to add favourites.</p>';
    return;
  }
  ...
  container.innerHTML = meals
    .map((meal) => {
      ...
      const editDeleteActions = meal.isCustom
        ? `<button class="btn btn-outline btn-xs" onclick="openRecipes(); editCustomRecipe('${meal.id}')">Edit</button><button class="btn btn-outline btn-xs danger-btn" onclick="deleteCustomRecipe('${meal.id}')">Delete</button>`
        : '<span class="tag-pill">Built-in</span>';
      const reviewButton = getCurrentUser()
        ? `<button class="btn btn-outline btn-xs" onclick="openReviewModalFromEncoded('${encodedMealNameForHandler}')">Review</button>`
        : "";
      return `
        <div class="recipe-card favourite-card">
...
function saveCustomRecipe() {
  ...
  const recipeId = editId || `custom-${Date.now()}`;
  ...
  const saveAsFavourite = Boolean(document.getElementById("recipe-favourite")?.checked);
  ...
  const saved = saveCurrentUserData((user) => {
    ...
    if (saveAsFavourite && !user.favouriteRecipeIds.includes(recipeId)) {
  ...
  resetRecipeForm();
  renderRecipeLibrary();
  if (document.getElementById("favourites-view")?.classList.contains("active")) renderFavourites();
}
...
function resetRecipeForm() {
  ...
  const favouriteInput = document.getElementById("recipe-favourite");
...
function editCustomRecipe(recipeId) {
  ...
  const favouriteInput = document.getElementById("recipe-favourite");
  if (favouriteInput) favouriteInput.checked = isFavourite(recipe.id);
...
function deleteCustomRecipe(recipeId) {
  ...
  saveCurrentUserData((currentUser) => {
    ...
    currentUser.favouriteRecipeIds = (currentUser.favouriteRecipeIds || []).filter((id) => id !== recipeId);
    currentUser.excludedRecipeIds = (currentUser.excludedRecipeIds || []).filter((id) => id !== recipeId);
  });
  resetRecipeForm();
  renderRecipeLibrary();
  if (document.getElementById("favourites-view")?.classList.contains("active")) renderFavourites();
...
function renderRecipeLibrary() {
  ...
  const favouritesOnly = Boolean(document.getElementById("recipe-library-favs-only")?.checked);
  let meals = getAllMealItems();

  if (type !== "all") meals = meals.filter((meal) => meal.type === type);
  if (favouritesOnly) meals = meals.filter((meal) => isFavourite(meal.id));
  if (query) meals = meals.filter((meal) => mealSearchText(meal).includes(query));
  ...
  container.innerHTML = meals
    .map((meal) => {
      ...
      const editDeleteActions = meal.isCustom
        ...
        : '<span class="tag-pill">Built-in</span>';
      const excluded = isRecipeExcluded(meal);
      const excludeButton = getCurrentUser()
        ? `<button class="btn btn-outline btn-xs" onclick="toggleRecipeExclusion('${meal.id}')">${excluded ? "Include" : "Exclude"}</button>`
        : "";
      const reviewButton = getCurrentUser()
        ? `<button class="btn btn-outline btn-xs" onclick="openReviewModalFromEncoded('${encodedMealNameForHandler}')">Review</button>`
        : "";
      const favouriteButton = `<button class="btn btn-outline btn-xs ${isFavourite(meal.id) ? "fav-active-btn" : ""}" onclick="toggleFavourite('${meal.id}')">${isFavourite(meal.id) ? "★ Favourited" : "☆ Favourite"}</button>`;
      const excludedBadge = excluded ? '<span class="tag-pill warning-pill">Excluded</span>' : "";
      return `
        <div class="recipe-card ${isFavourite(meal.id) ? "favourite-card" : ""} ${excluded ? "recipe-excluded" : ""}">
          <div class="recipe-card-top">
            <span class="meal-label">${toTitleCase(meal.type)}</span>
            <div class="button-row compact-actions">${favouriteButton}${editDeleteActions}${excludeButton}${reviewButton}</div>
          </div>
          <h3>${safeName}${excludedBadge}</h3>
          <p class="meal-meta">${formatMoney(meal.cost)} · ${formatCalories(meal.calories)} · ${formatWeightFromGrams(getMealPortionGrams(meal))} per serving</p>
          <p class="meal-meta review-summary">${escapeHTML(getReviewSummary(meal.name))}</p>
          <p class="meal-meta">Ingredients: ${escapeHTML(ingredients || "Not listed")}</p>
        </div>
...
function parseLeftoverText(text) {
  return text
    .split(",")
    .map((item) => item.trim().toLowerCase())
    .filter(Boolean);
...
function loadLeftoverIngredients() {
  const input = document.getElementById("leftover-input");
  if (!input) return;
  const user = getCurrentUserData();
  input.value = (user?.leftoverIngredients || []).join(", ");
}

function saveLeftoverIngredients() {
  const input = document.getElementById("leftover-input");
  if (!input) return;
  const leftovers = parseLeftoverText(input.value);
  const saved = saveCurrentUserData((user) => {
    user.leftoverIngredients = leftovers;
  });
  if (!saved) {
    showError("leftover-error", "Please log in before saving leftovers.");
    return;
  }
  showSuccess("leftover-success", leftovers.length ? "Leftover ingredients saved." : "Leftover list cleared.");
  renderLeftoverSuggestions();
}

function getLeftoverMatches(meal, leftovers) {
  const ingredients = (meal.ingredients || []).map((item) => item.name.toLowerCase());
  return leftovers.filter((leftover) =>
    ingredients.some((ingredient) => ingredient.includes(leftover) || leftover.includes(ingredient))
  );
}

function renderLeftoverSuggestions() {
  const container = document.getElementById("leftover-suggestions");
  if (!container) return;

  const user = getCurrentUserData();
  const leftovers = user?.leftoverIngredients || [];
  if (leftovers.length === 0) {
    container.innerHTML = '<p class="empty-state small-empty">Enter leftover ingredients to see recipe suggestions.</p>';
    return;
  }

  const preferences = user?.preferences || { dietary: [], allergies: [] };
  const suggestions = getAllMealItems()
    .filter((meal) => !isRecipeExcluded(meal))
    .map((meal) => ({ meal, matches: getLeftoverMatches(meal, leftovers) }))
    .filter((item) => item.matches.length > 0 && mealMatchesPreferences(item.meal, preferences, true))
    .sort((a, b) => b.matches.length - a.matches.length || a.meal.name.localeCompare(b.meal.name));

  if (suggestions.length === 0) {
    container.innerHTML = '<p class="empty-state small-empty">No safe recipe suggestions found for these leftovers.</p>';
    return;
  }
  ...
  container.innerHTML = suggestions
    .slice(0, 8)
    .map(({ meal, matches }) => `
      <div class="recipe-card leftover-card">
        <div class="recipe-card-top">
          <span class="meal-label">${toTitleCase(meal.type)}</span>
          <span class="tag-pill">${matches.length} match${matches.length > 1 ? "es" : ""}</span>
        </div>
        <h3>${escapeHTML(meal.name)}</h3>
        <p class="meal-meta">Uses: ${escapeHTML(matches.join(", "))}</p>
        <p class="meal-meta">${formatMoney(meal.cost)} · ${formatCalories(meal.calories)}</p>
        <div class="button-row compact-actions leftover-actions">
          <button class="btn btn-outline btn-xs" onclick="openReviewModalFromEncoded('${encodeRecipeNameForHandler(meal.name)}')">Review</button>
          <button class="btn btn-outline btn-xs" onclick="toggleRecipeExclusion('${meal.id}')">${isRecipeExcluded(meal) ? "Include" : "Exclude"}</button>
        </div>
      </div>
    `)
    .join("");
